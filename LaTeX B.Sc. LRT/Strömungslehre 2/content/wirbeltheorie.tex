\setcounter{page}{1}
\centering
\section*{\myul{Wirbeltheorie}}

\begin{tikzpicture}[remember picture, overlay]
% ---- ERSTE REIHE: Am Seitenursprung starten
\setlength{\rowheight}{52mm} % Erste Reihenhöhe: 54mm
...
\setlength{\rowheight}{28mm} % Zweite Reihenhöhe
    ...
    \Tile{A2}{([yshift=-10mm]A1.south west)}{\cellwidth}{\rowheight}
        {
            \titlerm{Allgemein} \\[-1.5ex]
            \begin{equation*}
                \underline{\Omega} = 2\,\underline{\omega} = \text{rot}(\underline{v})
            \end{equation*}
            {\color{NavyBlue}(7.11) \rotatebox[origin=c]{270}{$\Lsh$}}
        }{0}{1}{1}{0} % Kanten: unten & rechts
    ...
    \Tile{B2}{(A2.north east)}{\cellwidth}{\rowheight}
        {
            \titlerm{Dichtebeständig} \\[3.0ex]
            \begin{tabularx}{\dimexpr\cellwidth-2\TileSep}{c|c} % Breite der tabularx = (Zellenbreite - 2 * Tile-Randabstand)
                \centering
                \begin{tabular}[c]{@{}c@{}}\myul{Allgemein}\\[3.0ex]$\underline{\Omega} = \nabla(\text{div}(\underline{\psi})) - \Delta(\underline{\psi})$\\[3.0ex]\end{tabular} & \begin{tabular}[c]{@{}c@{}}\myul{Ebene Platte}\\[3.0ex]$\Omega_z = -\Delta(\psi_z)$\\[3.0ex]\end{tabular}
            \end{tabularx}
        }{0}{1}{1}{0} % Kanten: unten & rechts
